...
class Config(BaseSettings):
    ...
    # ID администраторов — подмножество MODERATOR_IDS (ADMIN_IDS=123,456)
    admin_ids: list[int] = []
    ...
    def parse_moderator_ids(cls, v: object) -> list[int]:
        ...
        if not ids:
            raise ValueError("MODERATOR_IDS не может быть пустым")
    ...
    def validate_admin_subset(self) -> "Config":
        extra = set(self.admin_ids) - set(self.moderator_ids)
        if extra:
            raise ValueError(
                f"ADMIN_IDS содержит ID не из MODERATOR_IDS: {sorted(extra)}"
            )
        return self
