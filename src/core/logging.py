...
# Where the rotating log file goes. The default is <project>/logs, which is
# what a checkout run outside Docker wants. The image sets LOG_DIR=/logs so a
# mounted volume actually receives the file — with the default, the log would
# be written inside the container and vanish on every recreate.
