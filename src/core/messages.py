...
_DEFAULTS: dict[str, str | dict[str, str]] = {
    ...
    'LOCK_NOOP_SELF': '✏️ Этот пост редактируете вы — откройте чат с ботом.',
    ...
    'EDIT_CAPTION_PROMPT': 'Текущий текст — в сообщении выше: его можно скопировать, поправить и отправить.\nДоступно символов: {remaining} (с учётом тегов).\nИли /cancel для отмены.',
    'EDIT_CAPTION_PROMPT_EMPTY': 'Отправьте описание для поста.\nДоступно символов: {remaining} (с учётом тегов).\nИли /cancel для отмены.',
    'EDIT_CAPTION_TOO_LONG': 'Описание с тегами слишком длинное: {total} символов при лимите {limit}. Нужно убрать ещё {overflow}.',
    ...
    'TAGS_WIZARD_SUGGESTED_HINT': '💡 Автор предлагал теги: {tags}\nОтправь нужные из них сообщением, если хочешь добавить.',
    'TAGS_WIZARD_SUGGESTED_FUZZY': '#{raw} (похоже на #{guess})',
    ...
    'USER_ALREADY_UNBANNED': 'Пользователь уже разблокирован.',
    ...
    'TAG_PRESET_EDIT_TAG_PROMPT': '<b>{section}</b>\n\nТекущий tag: <code>#{tag}</code>\nОтправьте новый tag (# можно не писать). Несколько тегов через пробел станут одной кнопкой.',
    ...
    'TAG_PRESET_INVALID_TAG': 'Tag не распознан: нужен хотя бы один тег. # можно не писать.',
    ...
    'TOPIC_CARD_SUGGESTED_TAGS': '<i>Автор предложил теги: {tags}</i>',
    ...
    # ── Moderators screen (roles management UI) ────────────────────────
    'MODERATORS_MENU_TEXT': '<b>Модераторы</b>\n\nНажмите на пользователя, чтобы открыть карточку.',
    'MODERATORS_EMPTY_TEXT': '<b>Модераторы</b>\n\nСписок пуст.',
    'ROLE_LABEL_ADMIN': 'Администратор',
    'ROLE_LABEL_MODERATOR': 'Модератор',
    'ROLE_FROM_ENV_BADGE': '(из .env)',
    'MODERATOR_CARD_TEXT': '<b>{name}</b>\n\n<b>Роль:</b> {role}\n<b>Telegram:</b> {username}\n<b>ID:</b> <code>{telegram_id}</code>\n{granted_info}',
    'MODERATOR_REMOVE_CONFIRM_TEXT': 'Удалить {user} из модераторов?\n\nМодератор потеряет доступ к панели.',
    'MODERATOR_REVOKE_ADMIN_CONFIRM_TEXT': 'Снять с {user} права администратора?\n\nАдминистратор останется модератором.',
    'BTN_MODERATORS': 'Модераторы',
    'BTN_ADD_MODERATOR': 'Добавить модератора',
    'BTN_INVITE_LINK': 'Создать ссылку-приглашение',
    'BTN_ENTER_ID': 'Ввести telegram_id',
    'BTN_GRANT_ADMIN': 'Сделать админом',
    'BTN_REVOKE_ADMIN': 'Снять админа',
    'BTN_REMOVE_MODERATOR': 'Удалить из модераторов',
    'MODERATOR_SELF_ACTION_FORBIDDEN': 'Нельзя менять роль самому себе.',
    'MODERATOR_LAST_ADMIN_FORBIDDEN': 'Нельзя снять роль последнего администратора.',
    'MODERATOR_CONFIG_PROTECTED': '{user} назначен через .env — изменить его роль нельзя.',
    'MODERATOR_ALREADY_MODERATOR': '{user} уже модератор.',
    'MODERATOR_TARGET_BANNED': '⛔ {user} заблокирован. Сначала разблокируйте его, чтобы выдать роль.',
    'MODERATOR_INVALID_ID': 'Некорректный telegram_id. Введите положительное целое число.',
    'MODERATOR_ENTER_ID_PROMPT': 'Отправьте <code>telegram_id</code> пользователя, которого нужно назначить модератором.',
    'MODERATOR_INVITE_LINK_TEXT': 'Ссылка-приглашение готова:\n\n{link}\n\nДействует {ttl_hours} ч и может быть использована только один раз.',
    'MODERATOR_INVITE_INVALID': 'Ссылка недействительна или уже использована.',
    'MODERATOR_INVITE_WELCOME': '🎉 Поздравляем, {user}! Теперь вы модератор бота.',
    'MODERATOR_ROLE_REMOVED_NOTICE': 'С вас снята роль модератора.',
    'MODERATOR_ADMIN_GRANTED_NOTICE': 'Вам выданы права администратора.',
    'MODERATOR_ADMIN_REVOKED_NOTICE': 'С вас сняты права администратора. Роль модератора сохранена.',
    'MODERATOR_INVITE_ALREADY_MODERATOR': '{user}, вы уже модератор.',
    'MODERATOR_INVITE_BANNED': '⛔ Вы заблокированы и не можете получить роль модератора.',
    'ADMIN_NOTIFY_MODERATOR_ADDED': '👤 {actor} назначил {user} модератором.',
    'ADMIN_NOTIFY_MODERATOR_REMOVED': '👤 {actor} снял {user} с роли модератора.',
    'ADMIN_NOTIFY_ADMIN_GRANTED': '👑 {actor} выдал {user} права администратора.',
    'ADMIN_NOTIFY_ADMIN_REVOKED': '👑 {actor} снял с {user} права администратора.',
    ...
    'MANAGEMENT_MODERATORS_LOCKED': '🔒 Список модераторов сейчас редактирует {mod}. Попробуйте позже.',
