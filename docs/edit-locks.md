...
- **Статуса `in_review` не существует.** Открытие поста по `/start review_<id>` берёт лок и перерисовывает карточку в топике, но `submissions.status` остаётся `pending` — «занято прямо сейчас» живёт исключительно в `edit_locks`. Попытка завести отдельный статус занятости разъедет очередь, дашборд и подсчёты, которые фильтруют по `pending`.
- **Edit-локи защищают только от гонки двух РАЗНЫХ модераторов.** Локи ключуются на `moderator_id`, поэтому двойной клик одного и того же модератора проходит `extend_lock` дважды без препятствий — от него защищает исключительно атомарный переход состояния (`transition_submission_status` и соседние guard-запросы): проигравший гонку получает `False` и не выполняет ни одного побочного эффекта.
