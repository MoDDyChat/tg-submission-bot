...
- On bot startup, `ensure_general_topic_nav()` creates or reuses a pinned navigation message with the status legend in the forum's General topic
...
2. Enters text → the bot forwards it to the viewer tagged «Сообщение от модератора по поводу поста #N» (Message from a moderator about post #N)
3. The viewer replies with **reply** text to the bot's message → the `IsModeratorReply` filter intercepts the reply → the bot extracts `sub_id` from the text (regex `msg.VIEWER_REPLY_PATTERN` from `core/messages.py`) and forwards the message to all moderators in `config.moderator_ids`
4. If the viewer replies with a media file, the bot rejects it with a hint (media replies are not supported)
5. When the media composition changes, `TOPIC_NOTIFY_MEDIA_CHANGED` is sent to the forum topic (🖼 Состав медиа изменён модератором — media composition changed by moderator)
6. All text messages are saved in the `messages` table
