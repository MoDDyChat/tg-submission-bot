...
4. **The post is published to the moderator forum group's topic**: each viewer has their own topic in `MODERATOR_GROUP_ID`; for a media post — media + a status card with an «Редактировать» (Edit) button; for text-only — just a text card
   - The post itself (and its media rows) is committed **before any Telegram call** — `submit_single_media`, `submit_text` and `_finalize_media_group` (`services/submission_intake.py`) `commit()` right after `create_submission`/`add_media`, then reply to the author, then hand off to `_publish_submission_to_topic`. From there each step is its own short transaction: `ensure_user_topic` + commit → `post_submission_card` + commit → `request_topic_title_sync` + commit. This is the incident behind post #377: one open transaction spanning both the domain insert and the Telegram calls hit `statement_timeout`/`lock_timeout` on `user_topics` mid-flight and lost the post along with an orphaned card. Now a failure at the topic/card step rolls back only that step: the post stays in the DB without `topic_card_message_id`, the author receives no additional message, and `topic_cards_recover_job` delivers the card later (or a manual Recover does). This is intentional: an error message would contradict the acceptance confirmation just sent and prompt the author to take an action that is not required. The post creation itself is never at risk. A failure at the title-sync step only logs a warning; the drift is caught by `topic_titles_reconcile_job`
...
**Card recover:** `topic_cards_recover_job` runs every minute and picks up active posts older than `min_age` (2 min) whose `topic_card_message_id` is still `NULL` (`list_active_submissions_without_card`, no `LIMIT` — a chronically failing post must not hide forever behind a head cap) — the case where the card was never sent in the first place, typically because `_publish_submission_to_topic` failed on the topic/card step during intake. Each post is reposted via the shared `_repost_card` helper straight away, no probe request and no artificial cap on the batch size, since a missing card is already a certain fact and doesn't need Telegram-side confirmation first. The shared `_recover_guard` (an `asyncio.Lock`) protects both this job and the manual **Recover постов** action; after the fresh DB read, the in-flight guard is checked before reposting so intake and recovery cannot duplicate a card within the same process. `min_age` is a second line of defense for an intake process that was interrupted: it gives the in-flight work time to finish. The worst-case recovery delay is about 3 min (2 min of age plus the next one-minute tick), instead of about 15 min.

If Telegram flood control (`TelegramRetryAfter`) is hit during a cardless pass, the pass stops and waits for the next tick. A tick never sleeps, so `max_instances=1` does not turn the recovery job into a queue of overlapping or already-stale runs. Manual Recover still has its own job to do: it also probes posts that *do* have a `topic_card_message_id` to catch the case where a card was sent but later vanished from the topic — `topic_cards_recover_job` doesn't do that, it only fills in cards that were never sent.

No separate guard is needed for the race where an author cancels while the card is still being posted. `topic_cards_reconcile_job` re-renders terminal posts no older than 48 h with `markup=None` (`src/services/topics.py:1155-1175`), so a canceled post's card loses its buttons automatically within 10 min.

There is a known restart limitation: if the container restarts after the media block was sent but before the card was sent, the already-sent media remains in the topic because `media_ids` live only in memory, and recovery reposts the block. The complete fix would persist `topic_media_message_ids` in the DB before sending the card; that is deliberately out of scope for now.
...
**Queue board:** `_render_queue_inner` / `_render_schedule_inner` commit the read-only phase (queue rows, chunks, schedule text and their checksums) before the first Telegram call. `message is not modified` counts as success and **the checksum gets written** to `system_messages` — otherwise every following pass would hit the same no-op edit again. An extra chunk is removed from `system_messages` only if Telegram confirms the deletion or the message is already gone (`message to delete not found`); on any other error the row is left in place and the deletion is retried on the next pass. `TelegramRetryAfter` is not handled per chunk: it ends the pass, `render_queue_tick` keeps the board dirty and skips ticks until `retry_after` has passed — firing the remaining chunks into an active flood only stretches it. The 5-minute self-heal edits changed chunks plus **one** unchanged chunk (round-robin), not every chunk: a no-op edit still spends the group's shared `EditMessageText` quota, and a full pass of ~15 chunks coinciding with a publication tripped flood control. Handlers answer the callback right after the durable commit, before card/notification calls that can wait out flood control — a late answer fails with `query is too old`.
...
**Resilience:** errors sending to the forum propagate to the calling code (`post_submission_card` is a critical operation). The already committed post remains without a `topic_card_message_id` and is handled by card recovery; no additional error message is sent to the viewer. Posts without a `topic_card_message_id` — update/delete are no-ops.
