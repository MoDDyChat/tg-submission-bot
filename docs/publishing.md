...
- `dead_at IS NULL` means alive; a non-NULL `dead_at` means the publication was overdue by more than 24h with no job scheduled for it. `services/dashboard.py` counts dead publications separately from the `scheduled` bucket (`scheduled - dead`) and shows them as «С ошибкой», and `topics_queue._build_schedule_lines` pulls dead publications out of the normal day-block layout into a dedicated block at the top of the schedule post (`SCHEDULE_DEAD_HEADER` + one `SCHEDULE_DEAD_LINE` per item), since they carry their own overdue date rather than an upcoming one.
