...
└── src/
    ...
    │   ├── topics_queue.py       # Queue board: dirty-marker + 60s coalescing tick with 5min self-heal; builds/updates general:queue:NN messages in the General topic; idempotency via MD5; asyncio.Lock against concurrent calls
...
- **Why the separate coalescing:** `editMessageText` in the moderator group is a single shared Telegram quota with the queue board (`services/topics_queue.py`). The General-topic queue has its own dirty flag as well as the author card and dashboard, and intake marks that flag instead of rendering the queue directly. Hanging the dashboard, author card, or General-topic queue on one event stream would multiply that traffic on every submission-status change and risk `TelegramRetryAfter` on bursts. Routing each source through its own dirty flag and independent minimal-interval job caps the added edit rate regardless of how many events fire in between ticks.
